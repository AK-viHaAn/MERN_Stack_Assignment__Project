...
const ManageTask = () => {
    ...
    const GetAllTasks = () => {
        axios.get("https://mern-stack-vi.netlify.app/.netlify/functions/api/gettask")
    ...
    const dragDropped = (e, Status) => {
        ...
        let task = demo.filter((dem) => {
            if (dem.Title === Title) {
                dem.Status = Status
                axios.put("https://mern-stack-vi.netlify.app/.netlify/functions/api/updatetask", { Title, Status })
